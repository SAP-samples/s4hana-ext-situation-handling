...
public class ValidationExceptionMapperTest {
	...
	@Test
	public void testErrorResponseWithViolations() {
		...
		// When
		final ErrorResponse errorResponse = ValidationExceptionMapper.toErrorResponse(exception);
		
		// Then
		assertThat("errorResponse.code", errorResponse.getCode(), is(ConstraintViolationException.class.getName()));
		assertThat("errorResponse.message", errorResponse.getMessageText(), is(expectedMessage));

		// Then there are two causes
		assertThat("errorResponse.cause", errorResponse.getCause(), hasSize(violationMessages.length));
		
		// And their code is ValidationExceptionMapper.CAUSE_ERROR_CODE
		final List<String> causeCodes = errorResponse.getCause().stream()
		...
		final List<String> causeMessages = errorResponse.getCause().stream()
				.map(ErrorResponse::getMessageText)
				.distinct()
				.collect(Collectors.toList());
	...
	@Test
	public void testErrorResponseDoesNotContainViolationIfItIsNull() {
		...
		// Then
		assertThat("errorResponse.cause", errorResponse.getCause(), is(empty()));
	}
	
	@Test
	public void testErrorResponseDoesNotContainDuplicatedViolationMessages() {
		...
		assertThat("errorResponse.cause", errorResponse.getCause(), hasSize(1));
	...
	public void testWithRealInvalidBean() {
		...
		assertThat("messages of errorResponse.cause", 
				errorResponse.getCause().iterator().next().getMessageText(),
